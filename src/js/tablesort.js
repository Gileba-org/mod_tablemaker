	...
	fdTableSort = {
		...
		init: function (tableId) {
			...
			for (var k = 0, tbl; (tbl = tables[k]); k++) {
				...
				allRowArr = tbl.getElementsByTagName("thead").length
				...
				for (c = 0; c < rowArr[0].length; c++) {
					if (
						rowArr[0][c].getAttribute(fdTableSort.colspan) &&
						rowArr[0][c].getAttribute(fdTableSort.colspan) > 1
					) {
				...
				for (c = 0; c < workArr.length; c++) {
					...
					for (i = 0; i < rowLength; i++) {
						if (!workArr[c][i]) {
							...
							colspan =
								cel.getAttribute(fdTableSort.colspan) > 1 ? cel.getAttribute(fdTableSort.colspan) : 1;
							rowspan =
								cel.getAttribute(fdTableSort.rowspan) > 1 ? cel.getAttribute(fdTableSort.rowspan) : 1;
				...
				for (c = 0; c < workArr.length; c++) {
					for (i = 0; i < workArr[c].length; i++) {
						if (
							workArr[c][i].className.search("fd-column-") == -1 &&
							workArr[c][i].className.search("sortable") != -1
						)
						...
						if (workArr[c][i].className.match("sortable")) {
							...
							for (var cn = workArr[c][i].childNodes.length; cn--; ) {
								if (
									workArr[c][i].childNodes[cn].nodeType == 1 &&
									(workArr[c][i].childNodes[cn].className == "fdFilterTrigger" ||
										/img/i.test(workArr[c][i].childNodes[cn].nodeName))
								) {
									continue;
								}
								if (
									workArr[c][i].childNodes[cn].nodeType == 1 &&
									/^a$/i.test(workArr[c][i].childNodes[cn].nodeName)
								) {
									workArr[c][i].childNodes[cn].onclick = workArr[c][i].childNodes[cn].onkeydown =
										null;
								}
							...
							workArr[c][i].className = workArr[c][i].className.replace(
				...
				if ("active" in columnNumSortObj) {
					...
					if (
						showOnly &&
						(fdTableSort.tableCache[tbl.id].colStyle || fdTableSort.tableCache[tbl.id].rowStyle)
					) {
		...
		/*@cc_on/*@if(@_win32)removeClass:function(e,c){e.className=!c?"":e.className.replace(new RegExp("(^|\\s)"+c+"(\\s|$)")," ").replace(/^\s*((?:[\S\s]*\S)?)\s*$/,'$1')},@else@*/ removeClass:
			function (e, c) {
				e.className = !c
					? ""
					: e.className
		...
		prepareTableData: function (table) {
			...
			for (row = 0; row < numberOfRows; row++) {
				...
				if (
					...
					(tr.parentNode && tr.parentNode.tagName.toLowerCase().search(/thead|tfoot/) != -1)
				)
					continue;
				data[rowCnt] = [];
				tds = tr.cells;
			...
			var colStyle =
				table.className.search(/colstyle-([\S]+)/) != -1 ? table.className.match(/colstyle-([\S]+)/)[1] : false;
			var rowStyle =
				table.className.search(/rowstyle-([\S]+)/) != -1 ? table.className.match(/rowstyle-([\S]+)/)[1] : false;
			var iCBack =
				table.className.search(/sortinitiatedcallback-([\S-]+)/) == -1
					? "sortInitiatedCallback"
					: table.className.match(/sortinitiatedcallback-([\S]+)/)[1];
			var cCBack =
				table.className.search(/sortcompletecallback-([\S-]+)/) == -1
					? "sortCompleteCallback"
					: table.className.match(/sortcompletecallback-([\S]+)/)[1];
		...
		initSort: function (noCallback, ident) {
			...
			var lastPos =
				dataObj.pos && dataObj.pos.className ? dataObj.pos.className.match(/fd-column-([0-9]+)/)[1] : -1;
			...
			if (dataObj.thList.length > 1) {
				...
				(lastPos == fdTableSort.pos && !identical) ||
				(thNode.className.search(/sortable-keep/) != -1 && lastPos == -1)
			) {
				...
				if (!identical) {
					if (thNode.className.match(/sortable-(numeric|currency|date|keep)/)) {
						...
						thNode.className.search(/sortable-([a-zA-Z\_]+)/) != -1 &&
						thNode.className.match(/sortable-([a-zA-Z\_]+)/)[1] in window
					) {
			...
			if (
				...
				"tableInfo" in tablePaginater &&
				fdTableSort.tableId in tablePaginater.tableInfo
			) {
